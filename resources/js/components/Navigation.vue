<script setup lang="ts">
import { Link } from '@inertiajs/vue3';
import { Home, Menu, NotebookTabs, PiggyBank, X } from 'lucide-vue-next';
import { ref } from 'vue';
const menuToggle = ref(true);
const links = [
    {
        'icon': Home,
        'title': 'Home',
        'href': '/',
        'description': 'The gateway to the Millers Lechon experience. Take a look on our offerings and feedbacks from our previous customers.',
    },
    {
        'icon': PiggyBank,
        'title': 'About',
        'href': '/about',
        'description': 'The story of our heritage, our secret roasting process, and the commitment to quality that defines our Lechon.',
    },
    {
        'icon': NotebookTabs,
        'title': 'Menu',
        'href': '/menu',
        'description': 'A visual showcase of our offerings, from whole lechon and belly rolls to signature side dishes, including pricing and serving sizes.',
    },
]
</script>

<template>
    <div class="relative">
        <button class="flex gap-5 items-center hover:cursor-pointer relative z-20" @click="menuToggle = ! menuToggle">
            <div class="aspect-square p-3.5 rounded-full border-2 *:transition-all *:last:absolute ease-in-out border-stone-800/25 bg-amber-400 grid place-items-center">
                <Menu :class="menuToggle ? 'scale-100' : 'scale-0'"></Menu>
                <X :class="menuToggle ? 'scale-0' : 'scale-100'"></X>
            </div>

            <div class="font-bold uppercase leading-5 text-left *:transition-all ease-in-out">
                <p :class="menuToggle ? 'scale-0' : 'scale-100'">Close</p>
                <p :class="menuToggle ? '-translate-y-1/2' : 'translate-y-0'">Menu</p>
            </div>
        </button>

        <nav :class="menuToggle 
            ? 'scale-0 p-5 pt-24 fixed -top-5 left-1/2 -translate-x-1/2 w-full max-w-4xl transition-all ease-in-out'
            : 'scale-100 p-5 pt-24 fixed -top-5 left-1/2 -translate-x-1/2 w-full max-w-4xl transition-all ease-in-out'">
            <div class="bg-white flex flex-col lg:flex-row gap-10 rounded-3xl p-5 lg:p-10 border-2 border-zinc-200">
                <div>
                    <p class="font-semibold uppercase text-4xl leading-tight">Miller's <br/> Lechon</p>
                    <p class="font-semibold uppercase">Since 1974</p>
                </div>

                <div class="p-5 border-2 grow rounded-2xl bg-zinc-100/25">
                    <div v-for="link in links" v-bind:key="link.title" class="mb-5 last:mb-0 group">
                        <Link :href="link.href" class="flex flex-col lg:flex-row gap-5 items-start *:transition-all ease-in-out">
                            <div class="border-2 border-zinc-200 bg-white p-2.5 group-hover:bg-green-200 group-hover:border-green-300 rounded-2xl grid place-items-center">
                                <component :is="link.icon"></component>
                            </div>
                            
                            <div>
                                <p class="text-xl font-bold mb-1">{{ link.title }}</p>
                                <p class="w-full">{{ link.description }}</p>
                            </div>
                        </Link>
                    </div>
                </div>
            </div>
        </nav>
    </div>
</template>